 ...
 * gone by the time it runs. `contactId`/`contactInboxId`/`conversationIds`
 * intentionally have no FK — they reference deleted rows. If the contact is
 * re-created (same inbox + platform sourceId), the matching row is removed so
 * the returning contact keeps their history.
 */
export const messageCleanupModel = pgTable(
  "MessageCleanup",
  {
    ...
    // Kept as a plain column (no FK): the tombstone must outlive an inbox
    // disconnect so orphaned shard/hypertable rows stay traceable.
