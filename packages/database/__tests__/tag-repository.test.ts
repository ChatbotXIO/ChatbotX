...
// Recursively collects every bound-parameter value inside a Drizzle SQL
// condition (the object `and(eq(...), ...)` returns), so a test can assert a
// query is scoped by workspaceId without depending on the condition's exact
// internal shape.
function collectBoundValues(
  node: unknown,
  ...
  if (Array.isArray(chunks)) {
    for (const chunk of chunks) {
      values.push(...collectBoundValues(chunk, seen))
    }
  }
  return values
}
...
describe("tagRepository.create", () => {
  test("passes id, workspaceId, name, and folderId into insert .values()", async () => {
    const returning = vi
      .fn()
      .mockResolvedValue([
        { id: "generated-id", workspaceId: WS, name: "VIP", folderId: null },
      ])
  ...
  test("passes a real folderId through instead of defaulting to null", async () => {
    const returning = vi.fn().mockResolvedValue([{ id: "generated-id" }])
    const values = vi.fn().mockReturnValue({ returning })
    const insert = vi.fn().mockReturnValue({ values })
    const tx = { insert }

    await tagRepository.create(
      { workspaceId: WS, name: "VIP", folderId: "folder-1" },
      tx as never,
    )
...
describe("tagRepository.update", () => {
  test("scopes the update to the tag id and workspaceId, and returns the updated row", async () => {
    const returning = vi
      .fn()
      .mockResolvedValue([{ id: "tag-1", workspaceId: WS, name: "New Name" }])
    ...
    const result = await tagRepository.update(
      { id: "tag-1", workspaceId: WS, name: "New Name" },
      tx as never,
    )
    ...
    const boundValues = collectBoundValues(where.mock.calls[0]?.[0])
    expect(boundValues).toContain("tag-1")
    expect(boundValues).toContain(WS)
    expect(result).toEqual({ id: "tag-1", workspaceId: WS, name: "New Name" })
  })

  test("returns undefined when no row matches the id/workspaceId scope", async () => {
    ...
    const result = await tagRepository.update(
      { id: "ghost", workspaceId: WS, name: "New Name" },
      tx as never,
    )
...
describe("tagRepository.softDeleteMany", () => {
  ...
  test("sets deletedAt and returns matched ids", async () => {
    ...
    expect(collectBoundValues(where.mock.calls[0]?.[0])).toContain(WS)
