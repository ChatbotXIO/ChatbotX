 ...
 * inline `generateInstagramFacebookRedirectUri` call — see the sibling
 * `channels/instagram/route.ts` for why this can't stay inline.
 */
export async function GET(req: NextRequest) {
  ...
  await connectSessionService.updateReturnUrl({
    id: session.id,
    returnUrl: `/channels/instagram-facebook/select?session=${session.id}`,
  })
  if (nextAction.type !== "open_url") {
    throw new Error(
      `Unexpected connect next action for instagramFacebook: ${nextAction.type}`,
    )
  }
