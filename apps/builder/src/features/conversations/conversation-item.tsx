...
import { AssignedType, Contact, Conversation } from "@ahachat.ai/database"
import { formatDistanceToNow } from "date-fns"
import { CheckCircleIcon, UsersRoundIcon } from "lucide-react"
import { useMemo, useState } from "react"
import { Badge } from "@/components/ui/badge";
import { ConversationResource } from "@/features/conversations/schemas/get-conversations-schema";
    ...
    <>
      {
        conversation.contact.assignedType === AssignedType.User ? (
          <Avatar>
            ...
            <AvatarFallback>{conversation.contact.assignedUser?.name ?? " "}</AvatarFallback>
          </Avatar>
        ) : (
...
export default function ConversationItem({ conversation, isActive, onSelect }: ConversationItemProps) {
  const [hasSeen] = useState<boolean>(
    !!conversation.contactLastSeenAt && !!conversation.agentLastSeenAt && conversation.contactLastSeenAt > conversation.agentLastSeenAt
  )
  ...
  const getLatestSeenAt = (conversation: Conversation): Date | null => {
    ...
    return conversation.contactLastSeenAt > conversation.agentLastSeenAt ? conversation.contactLastSeenAt : conversation.agentLastSeenAt
  }
  ...
  return (
    <div className="px-2 py-1 w-full" onClick={() => onSelect()}>
      <Button
      ...
      >
        ...
        <div className="flex-1 overflow-hidden">
          <div className="flex justify-between">
            <span className="text-left font-semibold truncate">{contactFullName}</span>
            {!!unreadCount && (<Badge variant="destructive"
                                    className="rounded- px-1 font-normal">
                {unreadCount}
              </Badge>
            )}
          </div>
          <p className="text-left text-sm text-gray-600 w-full truncate">{conversation.latestMessage?.content}</p>
          <div className="flex gap-2 items-center">
            {latestSeenAt && (<div
              className="flex-1 text-gray-500 truncate text-xs text-right">{formatDistanceToNow(latestSeenAt, { addSuffix: true })}</div>)}
            {
              hasSeen ? (
