...
const LIST_PATH = "/workspaces/{workspaceId}/contacts/{contactId}/notes"
...
describe("contactNotesAuthenticatedAPI — access scoping", () => {
  ...
  test("does not restrict a super admin even with onlyAssignedContacts set", async () => {
    const handler = handlersByPath[LIST_PATH]
    await handler?.({
      input: { workspaceId: "workspace-1", contactId: "contact-1" },
      context: {
        ...
        workspace: { ownerId: "owner-1" },
        workspaceMember: {
          permissions: { onlyAssignedContacts: true, superAdmin: true },
        },
      },
    })
