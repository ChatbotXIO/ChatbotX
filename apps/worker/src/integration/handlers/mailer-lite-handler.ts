...
export const handleMailerLiteStep = async ({
  conversation,
  flowVersionId,
  step,
}: FlowStepProps<MailerLiteStepSchema>) => {
  const { id: conversationId, chatbotId, contactId } = conversation

  try {
    const [mailerLiteIntegration, contact, contactCustomFields, chatbotFields] =
      await Promise.all([
        prisma.integrationMailerLite.findFirst({
          where: { chatbotId },
        }),
    ...
    if (!mailerLiteIntegration) {
      throw new SdkException("MailerLite integration is missing")
    }
    ...
    for (const m of step.mergeFields || []) {
      ...
      if (value) {
        fields[m.mailerLiteField] = value
      }
    }
    ...
    const payload = {
      ...
      groupIds: step.groupId ? [step.groupId] : undefined,
      fields,
      status: step.type,
      autoresponders: step.autoresponders,
    }

    await integrationMailerLite.actions.addOrUpdateSubscriber({
      ctx: {
        auth: {
    ...
    logger.error(`[MailerLite] Error for ${conversationId}:`, error)
    await sendFlow({ conversation, flowVersionId, step }, false)
  }
}

const sendFlow = async (
  { conversation, flowVersionId, step }: FlowStepProps<MailerLiteStepSchema>,
  isSuccess: boolean,
) => {
