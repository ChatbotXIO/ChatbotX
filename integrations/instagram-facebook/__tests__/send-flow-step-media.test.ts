...
const ctx = {
  auth: {
    ...
    metadata: { igId: "ig-1", pageId: "page-1", version: "v23.0" },
  },
} as never
...
describe("instagram-facebook sendFlowStep — image/video steps", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  test("sends a button-less image as a bare attachment, never a media template (regression guard for 100/2534015)", async () => {
  ...
  // canonical quick replies were dropped on every step but sendQuickReply.
  test("keeps quick replies on a single-message image step", async () => {
    await sendFlowStep({
      ctx,
      data: buildData(
        {
